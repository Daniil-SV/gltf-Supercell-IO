...
class BinaryReader:
    ...
    def __init__(
        ...
        If endianness is not given, it is set to little endian.\n
    ...
    def read_str(self, encoding=None) -> str | None:
        size = self.read_uint16()
        if size and 0xFFFF > size:
            return self.read_raw_str(size, encoding)

        return None

    def read_raw_str(self, size=None, encoding=None) -> str:
    ...
    @overload
    def read_int64(self, count: int) -> tuple[int, ...]: ...
    ...
    @overload
    def read_uint64(self, count: int) -> tuple[int, ...]: ...
    ...
    @overload
    def read_int32(self, count: int) -> tuple[int, ...]: ...
    ...
    @overload
    def read_uint32(self, count: int) -> tuple[int, ...]: ...
    ...
    @overload
    def read_int16(self, count: int) -> tuple[int, ...]: ...
    ...
    @overload
    def read_uint16(self, count: int) -> tuple[int, ...]: ...
    ...
    @overload
    def read_int8(self, count: int) -> tuple[int, ...]: ...
    ...
    @overload
    def read_uint8(self, count: int) -> tuple[int, ...]: ...
    ...
    @overload
    def read_bool(self, count: int) -> tuple[bool, ...]: ...
    ...
    def read_bool(self, count=None) -> bool | Tuple[bool, ...]:
        ...
        """
        if count is not None:
            return tuple([val > 0 for val in self.read_uint8(count)])

        return self.read_uint8() > 0

    @overload
    def read_matrix(self, count: int) -> tuple[Matrix, ...]: ...
    ...
    def read_matrix(self, count=None) -> Matrix | Tuple[Matrix, ...]:
        ...
        """
        total = 16 * count if count is not None else 16
        values = self.read_float(total)

        matrices = tuple(
            Matrix([chunk[i : i + 4] for i in range(0, 16, 4)])
            for chunk in (values[i : i + 16] for i in range(0, total, 16))
        )

        return matrices if count is not None else matrices[0]

    @overload
    def read_float(self, count: int) -> tuple[float, ...]: ...
    ...
    @overload
    def read_half_float(self, count: int) -> tuple[float, ...]: ...
    ...
    @overload
    def read_norm_half_float(self, count: int) -> tuple[float, ...]: ...
    ...
    def read_norm_half_float(self, count=None) -> float | Tuple[float, ...]:
        ...
        """
        if count is not None:
            return tuple([val * 0.000030758 for val in self.read_int16(count)])
        return self.read_int16(count) * 0.000030758

    @overload
    def read_struct(
        self, cls: type[T], count: int, *args, **kwargs
    ) -> tuple[T, ...]: ...

    @overload
    def read_struct(self, cls: type[T], count: None = None, *args, **kwargs) -> T: ...

    def read_struct(
        self, cls: type[T], count: int | None = None, *args, **kwargs
    ) -> T | tuple[T, ...]:
